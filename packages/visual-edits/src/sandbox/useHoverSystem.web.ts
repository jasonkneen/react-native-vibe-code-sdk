/**
 * useHoverSystem hook - Web implementation
 *
 * Core hover detection and selection logic for web platform.
 * Handles:
 * - Visual highlighting of hovered/selected elements
 * - Click capture to select elements
 * - Sending selection data to the API via Pusher
 */

import Pusher from 'pusher-js'
import { useEffect, useState, useCallback } from 'react'
import type { HoverSystemOptions, HoverSystemResult, HoverSelectionData } from '../types'
...
export const useHoverSystem = ({
  ...
  const [selectedHighlightDiv, setSelectedHighlightDiv] = useState<HTMLDivElement | null>(null)
  ...
  const handleClick = useCallback(
    (e: MouseEvent) => {
      ...
      const elementData: HoverSelectionData = {
        elementId: hoveredElement.id || 'No ID',
        content: hoveredElement.textContent?.trim()?.slice(0, 200) || 'No content',
      ...
      console.log('[visual-edits] Element Clicked:', elementData)
      ...
      if (sandboxId) {
        ...
        const parentUrl =
          process.env.EXPO_PUBLIC_API_BASE_URL || 'https://capsule-ide.vercel.app'

        console.log(`[visual-edits] Sending to: ${parentUrl}/api/hover-selection`)
        ...
        fetch(`${parentUrl}/api/hover-selection`, {
          ...
          .then(() => {
            console.log('[visual-edits] Sent selection data via API')
          })
          .catch((err) => {
            console.error('[visual-edits] Failed to send selection data:', err)
          })
      }
    },
    [enabled, hoveredElement, sandboxId, selectedHighlightDiv],
  )
  ...
  const handleMouseMove = useCallback(
    (e: MouseEvent) => {
      ...
      if (!target || target === highlightDiv || target === selectedHighlightDiv) return
    ...
    [enabled, highlightDiv, selectedHighlightDiv, selectedElement],
  )
  ...
  // Create highlight divs on mount
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      pusherChannel.bind('pusher:subscription_succeeded', () => {
        console.log(`[visual-edits] Connected to Pusher channel: ${channelName}`)
      })
      ...
      console.error('[visual-edits] Failed to initialize Pusher:', error)
    }
  }, [sandboxId, enabled])

  // Attach/detach event listeners based on enabled state
  useEffect(() => {
    if (typeof window === 'undefined') return
  ...
  }, [enabled, handleMouseMove, handleClick, handleMouseLeave, highlightDiv, selectedHighlightDiv])
