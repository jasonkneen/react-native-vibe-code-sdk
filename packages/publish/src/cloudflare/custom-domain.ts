 ...
 *
 * For custom domains on capsulethis.app (or configured base domain):
...
import type { CustomDomainResult, DnsRecord } from '../types'
...
export async function addCustomDomain(
  ...
  try {
    ...
    console.log(`[Custom Domain] Status: ${result?.status ?? 'unknown'}`)
    ...
    return {
      success: true,
      customDomain,
      status: (result?.status ?? 'pending') as CustomDomainResult['status']
    }
  } catch (error: any) {
...
/**
 * Get the base domain used for custom domains
 */
export function getCustomDomainBase(): string {
  return CUSTOM_DOMAIN_BASE
}
