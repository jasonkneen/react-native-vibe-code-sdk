...
export default function PeopleSearchScreen() {
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <Text style={styles.subtitle}>Find professionals by role, company, or expertise</Text>
      ...
      <View style={styles.examplesContainer}>
        ...
        <TouchableOpacity onPress={() => setQuery('Director of sales operations in Chicago SaaS')}>
          <Text style={styles.exampleQuery}>• Director of sales in Chicago SaaS</Text>
        </TouchableOpacity>
      ...
      {loading && (
        <View style={styles.loadingContainer}>
          ...
          <Text style={styles.loadingText}>Searching 1B+ people profiles...</Text>
        </View>
      )}
            ...
            >
              <View style={styles.resultHeader}>
                ...
                <View style={styles.resultContent}>
                  ...
                  {result.score && (
                    <Text style={styles.resultScore}>Relevance: {(result.score * 100).toFixed(1)}%</Text>
                  )}
                </View>
              </View>
            </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  searchInput: { borderWidth: 1, borderColor: '#e2e8f0', borderRadius: 8, padding: 12, backgroundColor: 'white', marginBottom: 12, fontSize: 15, color: '#1e293b' },
  searchButton: { backgroundColor: '#6366f1', padding: 16, borderRadius: 8, alignItems: 'center', marginBottom: 16 },
  buttonDisabled: { opacity: 0.6 },
  searchButtonText: { color: 'white', fontWeight: '600', fontSize: 16 },
  examplesContainer: { backgroundColor: '#f1f5f9', padding: 12, borderRadius: 8, marginBottom: 16 },
  examplesTitle: { fontSize: 13, fontWeight: '600', color: '#475569', marginBottom: 8 },
  exampleQuery: { fontSize: 13, color: '#6366f1', marginBottom: 4, textDecorationLine: 'underline' },
  loadingContainer: { alignItems: 'center', marginTop: 32 },
  loadingText: { marginTop: 12, color: '#6366f1', fontSize: 14 },
  errorContainer: { backgroundColor: '#fee2e2', padding: 12, borderRadius: 8, marginTop: 16 },
  error: { color: '#ef4444', textAlign: 'center' },
  resultsContainer: { marginTop: 24 },
  resultsTitle: { fontSize: 18, fontWeight: '600', color: '#1e293b', marginBottom: 16 },
  resultCard: { backgroundColor: 'white', borderRadius: 12, padding: 16, marginBottom: 12, borderWidth: 1, borderColor: '#e2e8f0' },
  resultHeader: { flexDirection: 'row', alignItems: 'flex-start' },
  resultNumber: { fontSize: 16, fontWeight: '700', color: '#6366f1', marginRight: 12, minWidth: 24 },
  resultContent: { flex: 1 },
  resultTitle: { fontSize: 15, fontWeight: '600', color: '#1e293b', marginBottom: 6, lineHeight: 20 },
  resultUrl: { fontSize: 13, color: '#6366f1', marginBottom: 4 },
  resultScore: { fontSize: 12, color: '#10b981', marginTop: 2 },
});
\`\`\`
...
4. **User Experience**: Show loading states, provide example queries, make result URLs clickable.
5. **Privacy & Ethics**: Only use publicly available profile information, respect user privacy.
6. **Performance**: Debounce search input to avoid excessive API calls, cache recent results.
7. **Result Formatting**: Display person's name and title prominently, show relevance scores when available.
