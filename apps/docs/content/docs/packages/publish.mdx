---
title: "@react-native-vibe-code/publish"
description: Cloudflare Pages deployment and custom domain management for React Native Vibe Code
---
...
|----------|------|----------|-------------|
| `sandboxId` | `string` | Yes | E2B sandbox ID to deploy from |
| `projectId` | `string` | Yes | Project ID for tracking |
| `appName` | `string` | Yes | App name (used for deployment name) |
| `appPath` | `string` | No | Path to app in sandbox (default: `/home/user/app`) |
| `existingProjectName` | `string` | No | Cloudflare project name for updates |
| `useExactName` | `boolean` | No | Use exact app name without nanoid suffix |
| `existingCustomDomain` | `string` | No | Preserve this subdomain during updates |
