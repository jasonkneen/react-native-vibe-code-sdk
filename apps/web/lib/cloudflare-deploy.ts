...
/**
 * Deploy a web build to Cloudflare Workers using wrangler
 */
export async function deployToCloudflare(
  options: CloudflareDeployOptions
): Promise<CloudflareDeployResult> {
  const { sandboxId, projectId, appName, appPath = '/home/user/app', existingProjectName, useExactName, existingCustomDomain } = options
  ...
  if (existingProjectName) {
    ...
    deploymentName = `${sanitizedAppName}-${nanoid()}`
  }
  ...
  try {
    ...
    console.log(`[Cloudflare Deploy] Sandbox ID type: ${typeof sandboxId}`)
